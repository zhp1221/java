...
public class Generate {
	...
	public static void main(String[] args) {
		...
		FastAutoGenerator.create(
				...
				"root", "zhp.1221").globalConfig(it -> {
					it.author("zhp").fileOverride().disableOpenDir().outputDir(property + "/src/main/java");
				}).packageConfig(it -> {
					it.parent("com").moduleName("zhanghp").controller("ctrl").entity("dao.bean").service("service")
							.serviceImpl("service.impl").mapper("dao.mapper").xml("mapper.xml").pathInfo(Collections
									.singletonMap(OutputFile.mapperXml, property + "/src/main/resources/mapper/"));
				}).strategyConfig(it -> {
					it.addInclude("demo")

							.entityBuilder().enableLombok().naming(NamingStrategy.underline_to_camel)
							.columnNaming(NamingStrategy.underline_to_camel).controllerBuilder()
