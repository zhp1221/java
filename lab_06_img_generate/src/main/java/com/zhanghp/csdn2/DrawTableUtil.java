...
// String path = FileUtils.FILE_PATH;
...
// Map<Integer, List<Cell>> rows =
// headCells.stream().collect(Collectors.groupingBy(Cell::getRow));
...
// HashMap<Integer, Integer> contentRowHeight = getContentRowHeight(tableRowContents,
// table);
// //处理表头
// dealTableHead(rows, table, actualLastHeadRowColumnCell, headCells, headRealRowHead);
// // 生成表格各单元格内容对象
// List<Cell> contents = getTableContent(tableRowContents, actualLastHeadRowColumnCell,
// rows.size(), contentRowHeight);
// // 合并单元格
...
// table.getHeadCells().stream().reduce(ImmutableTriple.of(new ArrayList<>(), new
...
// Map<Integer, List<Cell>> collect =
// headCells.stream().collect(Collectors.groupingBy(Cell::getRow));
// collect.forEach((key, value) -> {
// List<Cell> rowCells =
// value.stream().sorted(Comparator.comparing(Cell::getColumn)).collect(Collectors.toList());
// int belongColumn = 0;
...
// */
// public static void dealTableHead(Map<Integer, List<Cell>> rows, Table table, List<Cell>
// actualLastHeadRowColumnCell, List<Cell> headCells,
...
// List<Cell> perRowCells =
// rows.get(row).stream().sorted(Comparator.comparing(Cell::getColumn)).collect(Collectors.toList());
...
// int offset = IntStream.range(1, column).map(index ->
// actualLastHeadRowColumnCell.get(index - 1).getWidth()).sum();
...
// int sum = IntStream.rangeClosed(cell.getRow(),
// rows.size()).map(headRealRowHead::get).sum();
// cell.setHeight(sum);
// }
// if (cellWidth <= 0) {
// throw new RuntimeException("行:" + cell.getRow() + " 列:" + cell.getColumn() +
// "宽度必须大于0");
// }
...
// */
// private static HashMap<Integer, Integer> getHeadRowHeight(Map<Integer, List<Cell>>
// rows, Table table) {
// return rows.keySet().stream().map(row1 -> {
// Integer cellRow = rows.get(row1).stream().map(cell ->
// StringUtils.countMatches(cell.getContent(), "\n")).max(Integer::compare).orElse(1);
...
// * @return 表格各单元格内容对象
// */
// public static List<Cell> getTableContent(List<List<String>> tableRowContents,
// List<Cell> actualLastHeadRowColumnCell,
...
// */
// private static HashMap<Integer, Integer> getContentRowHeight(List<List<String>>
// tableRowContents, Table table) {
...
// */
// private static Pair<Integer, Integer> dealRowHeight(List<List<String>>
// tableRowContents, Table table, int row) {
...
// * @param table 表格属性
// */
// public static BufferedImage starDrawTable(int headRow, List<Cell> tableCells, Table
// table, int tableHeight) throws IOException {
// int marginY = table.getMarginY();
// int marginX = table.getMarginX();
// Map<Integer, List<Cell>> allTableRows =
// tableCells.stream().collect(Collectors.groupingBy(Cell::getRow));
// int allRows = allTableRows.size();
// // 画布高度
// int imageHeight = tableHeight + marginY * 2;
// // 画布宽度
// int tableWidth = tableCells.stream().filter(cell -> cell.getRow() ==
// 1).mapToInt(Cell::getWidth).sum();
...
// for (Cell cell : rowCells) {
// graphics.drawLine(cell.getX(), tableHeight + marginY, cell.getX() + cell.getWidth(),
// tableHeight + marginY);
// }
// return;
// }
...
// graphics.drawImage(image.getScaledInstance(imageWidth, imageHeight,
...
// * @param cell 要绘制的单元格
// */
// private static void coreMethod(Graphics2D graphics, Font font, boolean lastCellInRow,
// Cell cell) {
// graphics.setColor(Color.black);
// //每个单元格左起横线
// graphics.drawLine(cell.getX(), cell.getY(), cell.getX() + cell.getWidth(),
// cell.getY());
// //每个单元格左起竖线
...
// * @param headCells 表头单元格
// */
// public static List<Cell> mergeCells(List<Cell> contents, int[] mergeColumns, List<Cell>
// headCells) {
...
// }
// Map<Integer, List<Cell>> collect =
// contents.stream().collect(Collectors.groupingBy(Cell::getColumn));
// List<Cell> toDel = new ArrayList<>();
// for (int mergeColumn : mergeColumns) {
// List<Cell> cells =
// collect.get(mergeColumn).stream().sorted(Comparator.comparing(Cell::getRow))
...
// Map<Integer, List<Cell>> rows =
// cells.stream().collect(Collectors.groupingBy(Cell::getRow));
// //取第一行的所有单元格
...
// // 按列升序排序
// List<Cell> perRowCells =
// firstRow.stream().sorted(Comparator.comparing(Cell::getColumn)).collect(Collectors.toList());
...
// List<List<Cell>> allSonCells = collect.stream().map(s -> getCellSonCells(cells,
